...
let currentContainer = document.getElementById('current-container');
let forecast = document.getElementById('five-day');
let icon =document.getElementById('iconContainer');

fetch ('https://api.openweathermap.org/data/2.5/weather?q=' + cityInput.value + '&appid=51e134b64a886c4af2649caf80c493b5')
    .then(response => response.json())
    .then(data =>{
        for(i=0; i<5; i++){
        document.getElementById('day' +(i+1) +'Min').innerHTML = 'Min:' +Number(data.list[i].main.temp_min(Math.round(( - 273.15) * 9 / 5 + 32) + "\xB0" + "F"));
        }
        for(i=0; i<5; i++){
        document.getElementById('day' +(i+1) +'Max').innerHTML = 'Max:' +Number(data.list[i].main.temp_max(Math.round(( - 273.15) * 9 / 5 + 32) + "\xB0" + "F"));
        }
        for(i=0; i<5; i++){
...
//     var apiKey = "f30dc0b71f772a037a522282770190be"; 51e134b64a886c4af2649caf80c493b5
